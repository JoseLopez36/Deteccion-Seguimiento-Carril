...
\begin{table}[H]
    \centering
    \caption{Servicios Docker que componen el sistema.}
...
Este nodo recibe cada \textit{frame} de la cámara frontal y ejecuta el pipeline de visión clásica descrito en la Sección~\ref{sec:carriles}. A partir de las líneas detectadas calcula el error lateral del vehículo respecto al centro del carril: primero en píxeles (\texttt{offset\_px}) y luego en metros, dividiendo por la longitud focal $f_x$ extraída del mensaje \texttt{camera\_info}. Los resultados se publican en:
\begin{itemize}
...
A lo largo del desarrollo se han cumplido los tres objetivos planteados al inicio. En primer lugar, el módulo de percepción detecta de forma robusta las marcas viales y estima la posición lateral del vehículo a partir de un pipeline ligero basado en OpenCV, sin recurrir a redes neuronales ni a aceleración por GPU. Este enfoque, validado previamente sobre vídeo real de carretera, demostró ser suficientemente fiable para alimentar el lazo de control. En segundo lugar, el controlador PID de dirección, sintonizado en MATLAB/Simulink sobre un modelo cinemático tipo bicicleta, proporciona una respuesta estable y con error estacionario nulo ante perturbaciones de posición. Por último, la integración de ambos módulos en CARLA mediante ROS2 y contenedores Docker ha dado lugar a un sistema completo capaz de conducir el vehículo de forma autónoma manteniéndolo centrado en su carril.

Un resultado especialmente relevante es la confirmación experimental de que las ganancias obtenidas en simulación con MATLAB/Simulink son directamente transferibles al entorno CARLA/ROS2: el sistema opera correctamente a 18~km/h y 30~km/h sin necesidad de reajustar los parámetros, lo que valida tanto el modelo empleado en la sintonización como la coherencia entre ambos entornos. La estructura de control en cascada, que desacopla la dinámica longitudinal de la lateral, y el diseño modular del software han resultado clave para abordar el problema de forma incremental y para facilitar la depuración de cada bloque por separado.

El trabajo presenta, no obstante, algunas limitaciones que abren líneas de mejora futuras. El detector basado en visión clásica es sensible a condiciones de iluminación adversas, marcas viales muy desgastadas o curvas de radio reducido, situaciones en las que un detector basado en aprendizaje profundo --entrenado, por ejemplo, con el dataset sintético generado en este trabajo-- podría aportar mayor robustez. Asimismo, el modelo cinemático empleado para la sintonización ignora los efectos dinámicos que aparecen a velocidades elevadas, por lo que un modelo dinámico permitiría extender el rango de operación del vehículo. Finalmente, la validación se ha realizado únicamente en simulación; el siguiente paso natural sería trasladar el sistema a una plataforma física para evaluar su comportamiento en condiciones reales.

En conjunto, el proyecto demuestra que es posible construir un sistema de mantenimiento de carril completo y reproducible combinando herramientas de código abierto, y sienta una base sólida sobre la que incorporar técnicas de percepción y control más avanzadas.
